...
### Added
- **Curated Lovelace dashboard tools** (`lovelace_dashboards`, `lovelace_dashboard`, `lovelace_create_dashboard`, `lovelace_update_dashboard`, `lovelace_save_config`, `lovelace_delete_dashboard`). The Home Assistant Lovelace surface is WebSocket-only (no REST — `HEAD /api/lovelace/config` is a 404) and previously reachable only through the generic `ws_read`/`ws_write` passthroughs, which expose no schemas, return whole configs verbatim, and let `lovelace/config/save` silently replace an entire hand-tuned dashboard. The new tools fix all three: the single-dashboard read is **summarised** (top-level keys + per-view card list with best-effort entity/statistic targets; `full: true` opts into the verbatim config with `size_bytes`/`truncated`), and `lovelace_save_config` implements the safe edit path — **read → patch (`ops`) → diff → write → re-read to verify** (or an explicit `config` + `confirm: "replace"`), with `dry_run` (returns the diff, runs without approval) and `expect_hash` (optimistic concurrency) — because HA's `config/save` is a blind full replace with no locking. Unknown top-level keys (e.g. Kiosk Mode's `kiosk_mode`) are preserved, YAML-mode dashboards are refused pre-flight, and `url_path` → `dashboard_id` resolution is handled server-side. `lovelace_delete_dashboard` needs no `config/delete` follow-up: deleting a storage dashboard already removes its stored config (`StorageLovelaceConfig.async_delete` → `store.async_remove`). `save_config` and `delete_dashboard` are `always_gate`; after approval the executor runs the same handler, so gated and immediate paths behave identically. Namespaced by the integration name (e.g. `home_assistant_lovelace_dashboard`).
- **Omada curated group-membership tools** (`omada_list_groups`, `omada_group_add_members`, `omada_group_remove_members`). Agents previously had to hand-roll `PATCH …/profiles/groups/{groupType}/{groupId}`, which produced three 405s on 2026-09-06 (the `{groupType}` segment is required, and the item route accepts PATCH/DELETE only). The read tool projects groups compactly (`groupId, name, type, count, members[{ip, mask, description}]`). The write tools implement **read → merge → PATCH → verify** server-side, because Omada's group PATCH is a **full `ipList` replace** (omitting existing members silently drops them) — `groupType` is derived from the group, never caller-supplied, and non-IP types (MAC/IPv6/etc.) are rejected with a clear message. Both writes are approval-gated (`always_gate`) and both accept the standard optional `session_id`/`execution_id` grouping fields. Also added a proper `array` MCP param type (`_TYPE_MAP['array'] = 'list'`) so membership lists validate as JSON arrays rather than objects.

### Fixed
- **Lovelace handler audit rows lost the caller context (session/agent).** The new `lovelace_*` tools ran their inner WS commands through a handler path that never forwarded `agent`/`session_id`/`execution_id`, so every row logged `Agent = ''` and no session — invisible in the Command Center Sessions view — while the REST/`mcp` path and the generic `ws_read`/`ws_write` path were correctly stamped. The handler layer now threads the caller context (`run_handler` → `_ws` → `ha_ws_exec`) for both the immediate and the post-approval execution paths (the latter attributes to the operator while retaining the original session).
- **A config-less Lovelace dashboard could not be written.** A freshly created storage dashboard has no saved config, and `lovelace/config` returns `config_not_found: No config found.` for it. The read step treated that as a hard error, so `save_config` always returned `ha_no_config` and nothing was written. The dashboard is now resolved first (unknown `url_path` → `ha_dashboard_not_found`) and a missing config is treated as `{}`, so `set_view(view="new")` and explicit full replaces initialise the first view. Config-less reads return an empty summary rather than an error, and an unknown `url_path` now maps to `ha_dashboard_not_found` for consistency with `update`/`delete` (`ha_config_not_found` retired).
- **A doomed gated write no longer consumes an approval.** Gating previously created the pending call before the handler read/patched/diffed, so an invalid op or an unknown/YAML delete target was queued, approved, and then failed. A read-only preflight now runs before the approval envelope is created and surfaces the error directly.
- **Diff/summary nits.** `cards_changed` no longer counts a pure append at the end as a changed card (changes are counted at aligned indices; a per-view signal moved to `views_changed`); a card op on a view-less config now hints to `set_view(view="new")` first; and markdown/content cards in the summary carry a first-line `preview` so index-based edits are unambiguous.
- **MCP sessions had vanished from the Command Center sessions carousel.** `list_recent_session_summaries` selected a non-existent `hostname` column from `requests` (it lives on `gateways`), so `/api/sessions/recent` returned **500** and the client's `refreshRecentSessions()` silently swallowed the failure and left the SSH-only fallback render. MCP sessions only appeared there before because the surfaced API rows carried a `session_id` in `requests` — removing that duplication exposed the latent bug. The summary query now joins `gateways` for the host label, and the client logs the fetch failure instead of silently returning.

### Security
- **Gateway endpoints now require authentication (Phase 1).** The gateway-facing API trusted the LAN. `GET /api/policy/{ip}` returned the full whitelist, regex and blacklist — plus any queued fleet command and the uninstall/rollback/update/freeze trigger flags — to **any** unauthenticated caller (it silently mutated that gateway's `last_seen`/`policy_sync` too); `GET /api/policy_changes` had no auth; `/api/ticket/{id}` and `/api/request_status/{id}` accepted no/optional tokens; and `POST /api/register` returned a valid 64-char gateway token for **any self-reported IP** — a token oracle that also defeated the policy endpoints (the policy lists are global, so any minted token unlocked them, and an unauthenticated caller could create gateway rows and trigger operator alerts). Now:
  - a shared gate `_require_gateway()` accepts a valid `X-Gateway-Token` that matches the URL/payload IP, or an authenticated dashboard session, and is applied to `/api/policy/{ip}`, `/api/poll/{ip}`, `/api/ticket/{id}` and `/api/request_status/{id}` (the latter two are also bound to the token's IP so one gateway cannot claim another's ticket);
  - `/api/policy_changes` requires a dashboard session (the UI now calls it with `authFetch`);
  - `/api/register` never discloses a known gateway's token (a re-register requires that gateway's own token or a session) and requires a valid, single-use **enrollment token** (`X-Enrollment-Token`) to enroll a **new** IP. The enrollment token is now consumed at `/api/register` (the authoritative step) rather than when the bootstrap is served, so a slow install can finish; the served bootstrap forwards it to the installer, which sends it on registration. `/api/version` and `/api/cmd-descs` remain public by design.
  - **Phase 2 (next):** the remaining write endpoints (`/api/request`, `/api/log`, `/api/poll` writes, `/api/gateways/{ip}` DELETE, window-execute) and `/api/gateway-heartbeat`. The heartbeat needs the logger to start sending `X-Gateway-Token` first (it currently sends none), shipped via the fleet deploy.

### Changed
- **Tools can be backed by a multi-step handler.** Added an `integration_tools.handler` column and a `core/tool_handlers.py` registry, dispatched by `core.tool_runner.run_tool` for both read and mutating tools and by the approval executor in `main.approve_integration_call`. This is the WS equivalent of the HTTP `transform` escape hatch — the declarative row model executed exactly one upstream call, so composite tools (read → write → verify) could not be expressed. Handlers run commands through `ha_ws_exec`, which returns the **raw** (un-scrubbed) result so a config read → write → read-back round-trip can never reintroduce a `[redacted]` placeholder, while the audit row still stores only a scrubbed summary. Also fixed a latent WS bug: a WS read tool carrying a `transform` returned the raw body unshaped because `tool_runner` assumed the HTTP path had already run it.
- **`ha_seed` now forwards the full tool-field set** (`transform`, `handler`, `always_gate`, `strip_envelope`, `version`, `error_codes`, `path_variants`, `response_hint`) to `create_tool`/`update_tool`, matching the other seed catalogs — previously a Home Assistant tool declaring any of these silently lost it.
- **History tabs are now strictly segregated — API/MCP calls no longer appear under SSH Commands.** The two tabs are meant to separate SSH commands from proxied API calls, but resolved mutating API calls were also being duplicated into the `requests` table (an `integration-approved`/`integration-denied` row surfaced from the approve/deny endpoints), so they showed in the SSH tab as "API Executed"/"API Denied". Surfacing is removed: approved/executed calls already live in the integration audit log (Proxied Calls) via their executor row, and a **denied** call — which never executes — now records its own `denied` audit row there (the deny endpoint writes it). The `requests` read paths (`get_all_requests`, `search_requests`, session summaries) also exclude the legacy `integration-approved`/`integration-denied` rows so historical entries stop leaking into the SSH tab. The now-dead client branches for those statuses were removed. Denied decisions remain in the audit timeline (`integration_call_denied`) as before.
- **Session view is now a one-stop detail.** New `GET /api/sessions/{sid}/detail` returns every SSH command **and** proxied API call for a session in one request. The session modal (opened from the carousel, JIT "View", or a Proxied Calls session link) renders a chronological timeline where each MCP call shows the **AI's `reason`**, the **masked request args**, the **response summary**, method/path/status/latency/bytes, execution id, and the **approval decision**; each SSH command shows its command, status/TTL, reason, risk/anomaly hints, and execution id. Long fields are laid out as bounded, scrollable blocks so nothing is hidden.
- **Proxied Calls now shows which calls needed approval — and what you decided.** `integration_calls` gained `approval` (`approved`/`denied`/`auto`), `decided_at`, `reason`, and a masked, size-capped `request_summary` (populated by the runner for auto-run writes, by the approve endpoint for operator-approved writes, and by the deny endpoint for rejections). The Proxied Calls table gained an **Approval** column with a badge (Approved / Denied / Auto / —) and a filter (`All / Approval required / Approved by operator / Denied by operator / Auto`). Args are masked via the tool's `redact` flags and secret-key scrubbing; credentials are never persisted.
- **Pulse integration upgraded to v6.3.2.** The upstream API moved per-resource cpu/mem/disk under `metrics.{cpu,memory,disk}` (`{value, used, total, percent, unit}`, **no `free`** — avail is now computed as `total - used`), `/api/resources` returns a `data[]` envelope, resource types renamed (`container` → `system-container`, new `agent`/`physical_disk`), and platform identity relocated from `platformData` to `identity`/`canonicalIdentity`/`proxmox`. All pulse transforms re-projected: `fleet_summary` (now reads `data[]`, emits `aggregations` on `full`), `get_resource` (v6 returns the item directly), `list_storage` (reads `data[]`, metrics-based, `free` computed). `connection_health` reflects the new `host-<uuid>` key scheme. Alert ack tools now send the v6 body fields (`alertIdentifier` / `alertIdentifiers` instead of `id` / `ids`). **`list_backups` retired** — v6 has no backups endpoint (verified live: `/api/backups/unified`, `/api/backups`, `/api/pbsBackups`, `/api/pveBackups` all 404). The declared-but-unimplemented v6 tools now carry the **real manifest paths** (`/api/ai/patrol/…`, `/api/actions/{actionId}/decision`, `/api/resources/{resourceId}/operator-state` with the `resourceId`/`actionId` params) instead of guessed ones.
- **Policy what-if preview.** `POST /api/policies/preview` replays the last 7/30/90 days of distinct commands against a *draft* policy and reports how many would flip (`newly_blocked` / `newly_allowed` / `→ auto-approve` / `→ JIT`) with a top-50 sample. The verdict logic was factored into `core/policy_eval.py` (`evaluate_policy_verdict`); `/api/policies/test` now wraps it. Controls gained a "↻ Preview impact" modal (later folded into Save & Push, below).
- **Behavioural anomaly flags.** New `core/cmd_profiles.py` background scanner builds per-gateway base-binary profiles from a 30-day rolling window (excluding in-flight pending); `GET /api/requests` adds an `anomaly` field for pending rows. JIT tickets and History rows show **"🆕 first time this gateway runs X"** — with a ≥10-command grace period so a brand-new host isn't flagged for everything.
- **Policy editor UX.** "Save & Push Policies" now opens the impact preview as a **confirmation gate** (Cancel / Save & Push) instead of saving blindly. Policy chips render **newest-first**; the Tester shows an **"In lists"** membership line (Exact / Regex / Blocklist ✓); and a persistent **"unsaved changes"** banner appears until you push.
- **Unified Remove modal.** One "🗑 Remove" button per gateway opens a modal offering **Uninstall** (remote, existing flow) vs **Force remove from dashboard** (deletes only the dashboard record) — with an online/offline hint and a warning that a still-running gateway re-registers on its next poll. *(The underlying `DELETE /api/gateways/{ip}` force-remove endpoint already existed but was not wired to any UI.)*
...
### Fixed
- **Enrollment one-liner URL is now quoted.** The dashboard's copy button emitted `curl -s http://…/api/enroll?token=… | bash` unquoted — fine on bash, but zsh (TrueNAS SCALE's shell) glob-expands the `?` and aborts with "no matches found" before `curl` runs. The generated one-liner now wraps the URL in double quotes, and the README notes the gotcha.
...
- **Integrations & MCP — agent access to homelab APIs.** Eshu now exposes a second surface for AI agents: the **MCP server at `/mcp`** (Streamable HTTP) fronts curated integrations to your homelab APIs, applying the same human-in-the-loop model as the SSH gateway. The dashboard holds upstream credentials **server-side only** (never returned by any endpoint), every call is audited, read-only tools auto-run, and **mutating tools require operator approval** (pending queue in the dashboard; the agent polls `check_approval(id)`). Agent tokens are long-lived bearer tokens presented to `/mcp`; only a SHA-256 hash is stored. A **DNS-rebinding host allowlist** (**MCP Access**) controls which `Host` headers may reach the endpoint. Curated catalogs: **Proxmox** (`header`/PVE token), **Omada** (`oauth2` client-credentials with token TTL cache, `expiresIn`-driven, safety margin + automatic re-auth on HTTP 401 and Omada token-expiry errors −44112/−44113), **Home Assistant** (`bearer`, `call_service` mutating → approval), **Pulse** (trends/backups with >1MB truncation, `not_implemented` stubs), **Jellyfin** (`always_gate` mutations, error-code maps, `path_variants`, `response_hint`), **Pi-hole** (`query_token` auth, multi-instance via name-based namespacing), **Sonarr** & **Radarr** (`arr_seed` parameterized catalog, X-Api-Key auth), and **Prowlarr** (read projection only — indexer `fields[]` credentials are **never** surfaced; `indexer_stats` uses a superset field projection so per-indexer query/grabs/failures counts surface regardless of the API build's field names). Kinds with curated catalogs get **no generic read/write floor** (`NO_GENERIC_KINDS`); every other integration seeds a **generic `read`/`write`** pair for arbitrary REST — `read` now supports **HEAD** (`method` param), returning `{status, content_length, content_type, url}` metadata with no body (e.g. checking a media file's size without dumping bytes) and a stable `not_found` error for missing resources. Tools are **namespaced by integration name** (`mcp_server` prefix = sanitized integration name), so multi-instance setups can't collide. Tests are threaded-server based; a `ThreadingHTTPServer` change to the test mock servers removes a connection-reset flake under back-to-back OAuth2 retry sequences.
- **Secret handling hardening for the integration layer.** An **always-on response scrubber** redacts credentials from upstream bodies and error strings before they reach the agent or the audit trail — key-name-driven plus value patterns (`Bearer`, `AccessToken=`, `PVEAPIToken=`, JWTs, `auth=`, `X-Api-Key:`). Whole keys that carry real credentials are dropped from projections (list-shaped `fields[]`, e.g. Prowlarr indexer config) while HA service-schema `fields` dicts are preserved. **Pending approval rows persist the raw payload for execution but display masked** (suffix + scrub), and on approve/deny the payload is **stripped from the row**, keeping only a SHA-256 hash (`secret_hashes`); resolved integration calls are purged after **30 days** by a background cleanup loop. The integration secret is surfaced only as a last-4 `secret_suffix` in the UI/API.
